            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('admin.caffe.testimonial.index') }}"
                        class="bg-gray-200 text-gray-700 px-4 py-
                    ...
                    <div class="md:w-1/3 flex flex-col items-center">
                        @if ($testimonial->avatar)
                            <img src="{{ asset('storage/' . $testimonial->avatar) }}" alt="{{ $testimonial->name }}"
                                class="w-40 h-40 object-cover rounded-full shadow-lg border-4 border-white">
                        @else
                            <span
                                class="inline-flex items-center justify-center h-40 w-40 rounded-full bg-gray-200 text-gray-500 shadow-lg border-4 border-white">
                            ...
                            <div class="py-3 flex justify-between text-sm">
                                ...
                                <dd class="text-gray-900">
                                    @if ($testimonial->is_visible)
                                        <span
                                            class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Tampil
                                            di Website</span>
                                    @else
                                        <span
                                            class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Disembunyikan</span>
                                    @endif
                                </dd>
                            </div>
                            <div class="py-3 flex justify-between text-sm">
