...
@section('content')
    ...
    <head>
        ...
        <title>Detail Testimoni Salon</title>
        ...
        <div class="main-content flex-1 p-6">
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-[#292929]">Detail Testimoni Salon</h1>
                    <div class="flex space-x-2">
                        <a href="{{ route('admin.salon.testimonialsalon.edit', $testimonialsalon->id) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @if ($testimonialsalon->avatar)
                            <img src="{{ asset('storage/' . $testimonialsalon->avatar) }}"
                                alt="{{ $testimonialsalon->name }}" class="w-32 h-32 object-cover rounded-full shadow-lg">
                        @else
                            <span
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ $testimonialsalon->name }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Jabatan/Peran</label>
                            <p class="text-gray-700">{{ $testimonialsalon->role ?? '-' }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Rating</label>
                            @if ($testimonialsalon->rating)
                                <div class="flex items-center">
                                    <span
                                        class="text-yellow-500 font-semibold text-lg">{{ $testimonialsalon->rating }}</span>
                        ...
                        <div>
                            ...
                            @if ($testimonialsalon->is_visible)
                                <span
                ...
                <!-- Kutipan -->
                <div class="mt-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Kutipan (Quote)</label>
                    <div class="bg-gray-50 p-4 rounded-lg border-l-4 border-salon-pink">
                        <p class="text-gray-800 italic">"{{ $testimonialsalon->quote }}"</p>
                    </div>
                </div>
            </div>
        </div>
